...
const AIChat = () => {
  ...
  const searchKnowledgeBase = useCallback((query: string): KnowledgeBaseEntry[] => {
    ...
    const scored = knowledgeBase.map((entry: KnowledgeBaseEntry) => {
      ...
      const keywordsLower = entry.keywords?.map((k: string) => k.toLowerCase()) || [];
      
      for (const term of searchTerms) {
        if (questionLower.includes(term)) score += 3;
        if (answerLower.includes(term)) score += 1;
        if (keywordsLower.some((k: string) => k.includes(term))) score += 2;
      }
    ...
    return scored
      .filter((entry: KnowledgeBaseEntry & { score: number }) => entry.score > 0)
      .sort((a: KnowledgeBaseEntry & { score: number }, b: KnowledgeBaseEntry & { score: number }) => b.score - a.score)
      .slice(0, 3);
  }, [knowledgeBase]);
  ...
  const sendMessage = useCallback(async () => {
    if (!input.trim() || !userId || !currentSession) return;
    ...
    try {
      ...
      const relevantKnowledge = searchKnowledgeBase(currentInput);
      
      let context = '';
      if (relevantKnowledge.length > 0) {
        context = '📚 **Relevant Knowledge from our FAQ:**\n\n' + 
          relevantKnowledge.map((k: KnowledgeBaseEntry) => 
            `**Q: ${k.question}**\n**A:** ${k.answer}`
          ).join('\n\n');
      ...
      let prompt = conversationHistory.map((m: { role: string; content: string }) => 
        `${m.role === 'user' ? 'User' : 'Assistant'}: ${m.content}`
      ).join('\n');

      if (context) {
        prompt = `${context}\n\nBased on the above knowledge and the conversation history, respond to the user's latest message:\n\n${prompt}`;
      }
      ...
      try {
        const geminiResponse = await aiService.geminiChat(prompt);
        ...
        try {
          const groqResponse = await aiService.groqChat(prompt);
      ...
      if (relevantKnowledge.length > 0 && response) {
        const sources = relevantKnowledge.map((k: KnowledgeBaseEntry) => `- ${k.question}`).join('\n');
        response += `\n\n---\n📚 *Based on ${relevantKnowledge.length} knowledge base entries*`;
      }
  ...
  }, [input, userId, currentSession, messages, knowledgeBase, searchKnowledgeBase]);
